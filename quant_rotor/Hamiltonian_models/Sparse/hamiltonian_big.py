...
from quant_rotor.Hamiltonian_models.Sparse.hamiltonian import hamiltonian_sparse


def hamiltonian_big_sparse(
    state: int,
    site: int,
    g_val: float,
    ...
    matrix_p_to_natural_orbital_sparse = sp.csr_matrix(
        matrix_p_to_NO_full[:, index_d[:state]]
    )

    K_mu = (
        matrix_p_to_natural_orbital_sparse.T.conj()
        @ K
        @ matrix_p_to_natural_orbital_sparse
    )
    ...
    right = sp.kron(
        matrix_p_to_natural_orbital_sparse, matrix_p_to_natural_orbital_sparse
    )

    V_mu = left @ V @ right

    H_mu = hamiltonian_sparse(
