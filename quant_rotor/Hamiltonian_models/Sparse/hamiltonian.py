...
from quant_rotor.Hamiltonian_models.Sparse.support_ham import (
    H_kinetic_sparse,
    H_potential_sparse,
    build_V_in_p,
)
...
def hamiltonian_sparse(
    ...
    V_final = H_potential_sparse(state, site, V_in_p, 1)
    ...
    if curve_L:
        l = (1 / 2) * lambda_val
        L_sparce = sp.diags(
            [l, 0, l], offsets=[-1, 0, 1], shape=(state**site, state**site)
        )

        H_final = H_final + L_sparce
    if curve_R:
        l = (1j / 2) * lambda_val
        L_sparce = sp.diags(
            [l, 0, l], offsets=[-1, 0, 1], shape=(state**site, state**site)
