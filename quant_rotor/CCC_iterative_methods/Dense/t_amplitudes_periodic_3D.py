...
def t_periodic(
    ...
    while True:
        ...
        for x in range(1, site):
            ...
            tensors.t_ab_ij_tensor[x, 0] = tensors.t_ab_ij_tensor[0, x]
    ...
    if periodic:
        ...
        for y in range(dim):
            for x in range(dim):
                ...
                energy += (
                    np.einsum(
                        "ijab, abij->",
                        qs.v_term(i, i, a, a, site_y, site_x % site),
                        qs.t_term(site_y, site_x % site),
                    )
                    * 0.5
                )
                ...
                energy += (
                    np.einsum(
                        "ijpq, pi, qj->",
                        qs.v_term(i, i, p, p, site_y, site_x % site),
                        qs.B_term(i, site_y),
                        qs.B_term(i, site_x % site),
                    )
                    * 0.5
                )
        ...
        # energy calculations
        for site_y in range(site):
            energy += np.einsum("ip, pi->", qs.h_term(i, p), qs.B_term(i, site_y)) * 0.5
            ...
            for site_y in range(site):
                if site_y < site_y:
                    ...
                    energy += np.einsum(
                        "ijab, abij->",
                        qs.v_term(i, i, a, a, site_y, site_y % site),
                        qs.t_term(site_y, site_y % site),
                    )
                    # noinspection SpellCheckingInspection
                    energy += np.einsum(
                        "ijpq, pi, qj->",
                        qs.v_term(i, i, p, p, site_y, site_y % site),
                        qs.B_term(i, site_y),
                        qs.B_term(i, site_y % site),
                    )
