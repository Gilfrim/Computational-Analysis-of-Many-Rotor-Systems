...
def thermofield_change_of_basis(hamiltonian):
        ...
        for i in range(physical_hilbert_dim):
            for j in range(physical_hilbert_dim):
                ...
                if i == j and col < (primitive_hamiltonian.shape[0])**0.5:
                    change_of_basis_matrix[:, col][i * physical_hilbert_dim + j] = (
                        diagonal_basis[i, col]
                    )
                ...
                # Ensure there are no diagonal components as diagonal vectors in primitive tensor prod basis is given above
                    ...
                    if not np.any(
                        change_of_basis_matrix[:, col]
                    ):  # if column after diagonal vectors in tensor basis is zeros
                        ...
                        if (
                            counter > 0
                            and col % (primitive_hamiltonian.shape[0]) ** 0.5 == 0
                        ):
                            counter += 1
    ...
    return (change_of_basis_matrix, diagonal_vecs)
...
def thermofield_change_of_basis_new(hamiltonian, temperature):
    # place hamiltonian in tensor product basis
    d_pq = np.eye(hamiltonian.shape[0], hamiltonian.shape[1])
    primitive_hamiltonian = oe.contract(
        "pq,mw->pmqw", hamiltonian, d_pq, optimize="optimal"
    ).reshape(hamiltonian.shape[0] ** 2, hamiltonian.shape[0] ** 2)
    # primitive_hamiltonian = np.kron(hamiltonian,d_pq)
            ...
            for j in range(physical_hilbert_dim):
                ...
                # Ensure there are no diagonal components as diagonal vectors in primitive tensor prod basis is given above
                    ...
                    if not np.any(
                        ...
                        # add tensor prod basis vector in orginal basis set
                        change_of_basis_matrix[:, col][
                            i * physical_hilbert_dim + j + counter
                        ] = 1
...
def depr_H_tilde_maker(hamiltonian):
        ...
        for j in range(physical_hilbert_dim):
            for k in range(physical_hilbert_dim):
                if j == k:
                    ...
                    H_Tilde_2[i, k + physical_hilbert_dim * j] = (
                        diagonal_basis[j, i] * hamiltonian[j, k]
                    )
    ...
    h_4 = np.kron(
        d_tilde, hamiltonian
    )  # The element only exist if the hamiltonian is diagonal in tilde basis
