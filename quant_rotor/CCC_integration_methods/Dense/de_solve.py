...
def tdcc_differential_equation(
    ...
    for site_1 in range(1, sites):
        ...
        for site_2 in range(1, sites):
            tensors.t_ab_ij_tensor[site_2, (site_1 + site_2) % sites] = (
    ...
    for site_x in range(sites):
        ...
        for site_y in range(site_x + 1, site_x + sites):
            ...
            energy += (
                np.einsum(
                    "ijab, abij->",
                    qs.v_term(i, i, a, a, site_x, site_y % sites),
                    qs.t_term(site_x, site_y % sites),
                )
                * 0.5
            )
            # noinspection SpellCheckingInspection
            energy += (
                np.einsum(
                    "ijpq, pi, qj->",
                    qs.v_term(i, i, p, p, site_x, site_y % sites),
                    qs.B_term(i, site_x),
                    qs.B_term(i, site_y % sites),
                )
                * 0.5
            )
...
def integration_scheme(
    ...
    time, T_0, t1_max, t_2max, t_1, t_2 = postprocess_rk45_integration_results(
        sol, t0_stored, states, sites
    )
