...
from quant_rotor.CCC_iterative_methods.Sparse.t_amplitudes_sub_class_fast import (
    PrecalcalculatedTerms,
    QuantumSimulation,
    SimulationParams,
    TensorData,
)
from quant_rotor.Hamiltonian_models.Sparse.support_ham import build_V_prime_in_p
...
def tdcc_differential_equation(
    ...
    two_max = (np.array([i.toarray() for i in qs.tensors.t_ab_ij_tensor])).flat[
        np.argmax(np.abs(np.array([i.toarray() for i in qs.tensors.t_ab_ij_tensor])))
    ]
    ...
    double = np.array(
        [sp.csr_matrix((a, a), dtype=float) for _ in range(site)], dtype=object
    )
...
def integration_scheme(
    ...
    init_amps = np.concatenate(
        (double_temp.flatten(), single.toarray().flatten(), np.array([t_0]))
    )
