...
import Create_new_operators.writeMatrixElements as wME
...
file_path_folder = r"/Users/gilfrim/Desktop/Projects/Computational_Analysis_of_Many_Rotor_Systems/z_non_periodic"
...
with open(file_path_K, mode="r", newline="") as csvfile_h:
    ...
    for row_h in reader_h:
        if float(row_h[2]) != 0.0:
            h_dict[((int(row_h[0]) - max_shift), (int(row_h[1]) - max_shift))] = float(
                row_h[2]
            )
...
with open(file_path_V, mode="r", newline="") as csvfile_v:
    ...
    for row_v in reader_v:
        if float(row_v[4]) != 0.0:
            v_dict[
                (
                    (int(row_v[0]) - max_shift),
                    (int(row_v[1]) - max_shift),
                    (int(row_v[2]) - max_shift),
                    (int(row_v[3]) - max_shift),
                )
            ] = float(row_v[4])
...
if HF:
    ...
    if overlap_index_final != 0:
        fock_final_val[[0, overlap_index_final]] = fock_final_val[
            [overlap_index_final, 0]
        ]
        fock_final_vec[:, [0, overlap_index_final]] = fock_final_vec[
            :, [overlap_index_final, 0]
        ]
...
if __name__ == "__main__":
    ...
    with open(file_path_energy, "w", encoding="utf-8") as energy_file, open(
        file_path_t_1_amplitudes, "w"
    ) as t_1_amplitudes_file, open(
        file_path_t_2_amplitudes, "w"
    ) as t_2_amplitudes_file:
        ...
        while True:
            ...
            for site_1 in range(1, sites):
                ...
                for site_2 in range(1, sites):
                    t_ab_ij_tensor[site_2, (site_1 + site_2) % sites] = t_ab_ij_tensor[
                        0, site_1
                    ]
            ...
            for site_x in range(sites):
                energy += (
                    np.einsum("ip, pi->", h_term(i, p, site_x), B_term(p, i, site_x))
                    * 0.5
                )
                for site_y in range(site_x + 1, site_x + sites):
                    ...
                    energy += (
                        np.einsum(
                            "ijab, abij->",
                            v_term(i, j, a, b, site_x, site_y % sites),
                            t_term(a, b, i, j, site_x, site_y % sites),
                        )
                        * 0.5
                    )
                    # noinspection SpellCheckingInspection
                    energy += (
                        np.einsum(
                            "ijpq, pi, qj->",
                            v_term(i, j, p, q, site_x, site_y % sites),
                            B_term(p, i, site_x),
                            B_term(q, j, site_y % sites),
                        )
                        * 0.5
                    )
