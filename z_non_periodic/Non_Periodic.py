    ...
    for line in input_file:
        ...
        if key in expected_types:
            ...
            try:
                ...
                errors.append(
                    f"{globals()[expected_types[key][1]]}, {key}: couldn't convert to {expected_type.__name__}"
                )
    ...
    for line_testing in testing_file:
        ...
        if key_testing in expected_types:
            ...
            try:
                ...
                # if they don't match adds error to list of errors
                errors.append(
                    f"{globals()[expected_types[key_testing][1]]}, {key_testing}: couldn't convert to {expected_type.__name__}"
                )
...
if i % 2 == 0 or a % 2 != 0:
    raise ValueError(
        "Overlap between low and high stats, will cause divide by zero in denominator update"
    )
...
with open(r"z_non_periodic/matrix_elements_K.csv", mode="r", newline="") as csvfile_h:
    ...
    for row_h in reader_h:
        if float(row_h[2]) != 0.0:
            h_dict[((int(row_h[0]) - m_max_shaeer), (int(row_h[1]) - m_max_shaeer))] = (
                float(row_h[2])
            )
...
with open(r"z_non_periodic/matrix_elements_V.csv", mode="r", newline="") as csvfile_v:
    ...
    for row_v in reader_v:
        if float(row_v[4]) != 0.0:
            v_dict[
                (
                    (int(row_v[0]) - m_max_shaeer),
                    (int(row_v[1]) - m_max_shaeer),
                    (int(row_v[2]) - m_max_shaeer),
                    (int(row_v[3]) - m_max_shaeer),
                )
            ] = float(row_v[4])
    ...
    for v_axis_1 in range(p):
        for v_axis_2 in range(p):
            for v_axis_3 in range(p):
                v_full[v_axis_0, v_axis_1, v_axis_2, v_axis_3] = g * v_dict.get(
                    (
                        func.p_to_m(v_axis_0),
                        func.p_to_m(v_axis_1),
                        func.p_to_m(v_axis_2),
                        func.p_to_m(v_axis_3),
                    ),
                    0,
                )
...
t_ab_ij_tensor = np.full((sites, sites, a, b, i, j), 0, dtype=complex)

# t_a_i_tensor = np.random.rand(sites, a, i) + np.zeros((sites, a, i), dtype=complex)
# t_ab_ij_tensor = np.random.rand(sites, sites, a, a, i, i) + 1j * np.zeros(
#     (sites, sites, a, a, i, i), dtype=complex
# )
...
def v_term(
    ...
    if gap == True and (
        v_site_1 == gap_site
        and (v_site_2 == gap_site + 1)
        or (v_site_1 == gap_site + 1 and v_site_2 == gap_site)
    ):
...
def residual_single(x_s: int) -> np.array:
    ...
    R_single = np.zeros((a, i), dtype=np.complex128)

    R_single += np.einsum(
        "ap, pq, qi->ai", A_term(a, p, x_s), h_term(p, q, x_s), B_term(q, i, x_s)
    )
        ...
        if z_s != x_s:
            if i_method >= 1:
                ...
                R_single += np.einsum(
                    "ap, plcd, cdil->ai",
                    A_term(a, p, x_s),
                    v_term(p, l, c, d, x_s, z_s),
                    t_term(c, d, i, l, x_s, z_s),
                )
            # noinspection SpellCheckingInspection
            R_single += np.einsum(
                "ap, plqs, qi, sl->ai",
                A_term(a, p, x_s),
                v_term(p, l, q, s, x_s, z_s),
                B_term(q, i, x_s),
                B_term(s, l, z_s),
            )
...
def residual_double_sym(x_ds: int, y_ds: int) -> np.array:
    ...
    R_double_symmetric = np.zeros((a, b, i, j), dtype=np.complex128)

    if i_method >= 1:
        # noinspection SpellCheckingInspection
        R_double_symmetric += np.einsum(
            "ap, bq, pqrs, ri, sj->abij",
            A_term(a, p, x_ds),
            A_term(b, q, y_ds),
            v_term(p, q, r, s, x_ds, y_ds),
            B_term(r, i, x_ds),
            B_term(s, j, y_ds),
        )
        if i_method >= 2:
            # noinspection SpellCheckingInspection
            R_double_symmetric += np.einsum(
                "ap, bq, pqcd, cdij->abij",
                A_term(a, p, x_ds),
                A_term(b, q, x_ds),
                v_term(p, q, c, d, x_ds, y_ds),
                t_term(c, d, i, j, x_ds, y_ds),
            )
            # noinspection SpellCheckingInspection
            R_double_symmetric -= np.einsum(
                "abkl, klpq, pi, qj->abij",
                t_term(a, b, k, l, x_ds, y_ds),
                v_term(k, l, p, q, x_ds, y_ds),
                B_term(p, i, x_ds),
                B_term(q, j, y_ds),
            )
            if i_method == 3:
                ...
                if sites >= 4:
                    for z_ds in range(sites):
                        for w_ds in range(sites):
                            if (
                                z_ds not in {x_ds, y_ds}
                                and w_ds not in {x_ds, y_ds}
                                and z_ds != w_ds
                            ):
                                # noinspection SpellCheckingInspection
                                R_double_symmetric += np.einsum(
                                    "klcd, acik, bdjl->abij",
                                    v_term(k, l, c, d, z_ds, w_ds),
                                    t_term(a, c, i, k, x_ds, z_ds),
                                    t_term(b, d, j, l, y_ds, w_ds),
                                )
...
def residual_double_non_sym_1(x_dns_1: int, y_dns_1: int) -> np.array:
    ...
    R_double_non_symmetric_1 = np.zeros((a, b, i, j), dtype=np.complex128)

    if i_method >= 1:
        # noinspection SpellCheckingInspection
        R_double_non_symmetric_1 += np.einsum(
            "ap, pc, cbij->abij",
            A_term(a, p, x_dns_1),
            h_term(p, c, x_dns_1),
            t_term(c, b, i, j, x_dns_1, y_dns_1),
        )
        # noinspection SpellCheckingInspection
        R_double_non_symmetric_1 -= np.einsum(
            "abkj, kp, pi->abij",
            t_term(a, b, k, j, x_dns_1, y_dns_1),
            h_term(k, p, x_dns_1),
            B_term(p, i, x_dns_1),
        )
        ...
        if i_method >= 2:
            for z_dns_1 in range(sites):
                if z_dns_1 != x_dns_1 and z_dns_1 != y_dns_1:
                    # noinspection SpellCheckingInspection
                    R_double_non_symmetric_1 += np.einsum(
                        "acik, krcs, br, sj->abij",
                        t_term(a, c, i, k, x_dns_1, z_dns_1),
                        v_term(k, r, c, s, z_dns_1, y_dns_1),
                        A_term(b, r, y_dns_1),
                        B_term(s, j, y_dns_1),
                    )
                    # noinspection SpellCheckingInspection
                    R_double_non_symmetric_1 += np.einsum(
                        "bq, qlds, adij, sl->abij",
                        A_term(b, q, y_dns_1),
                        v_term(q, l, d, s, y_dns_1, z_dns_1),
                        t_term(a, d, i, j, x_dns_1, y_dns_1),
                        B_term(s, l, z_dns_1),
                    )
                    # noinspection SpellCheckingInspection
                    R_double_non_symmetric_1 -= np.einsum(
                        "abkj, lkrp, pi, rl->abij",
                        t_term(a, b, k, j, x_dns_1, y_dns_1),
                        v_term(l, k, r, p, z_dns_1, x_dns_1),
                        B_term(p, i, x_dns_1),
                        B_term(r, l, z_dns_1),
                    )
...
def residual_double_non_sym_2(x_dns_2: int, y_dns_2: int) -> np.array:
    ...
    R_double_non_symmetric_2 = np.zeros((b, a, j, i), dtype=np.complex128)

    if i_method >= 1:
        # noinspection SpellCheckingInspection
        R_double_non_symmetric_2 += np.einsum(
            "bp, pc, caji->baji",
            A_term(b, p, y_dns_2),
            h_term(p, c, y_dns_2),
            t_term(c, a, j, i, y_dns_2, x_dns_2),
        )
        # noinspection SpellCheckingInspection
        R_double_non_symmetric_2 -= np.einsum(
            "baki, kp, pj->baji",
            t_term(b, a, k, i, y_dns_2, x_dns_2),
            h_term(k, p, y_dns_2),
            B_term(p, j, y_dns_2),
        )
        ...
        if i_method >= 2:
            for z_dns_2 in range(sites):
                if z_dns_2 != x_dns_2 and z_dns_2 != y_dns_2:
                    # noinspection SpellCheckingInspection
                    R_double_non_symmetric_2 += np.einsum(
                        "bcjk, krcs, ar, si->baji",
                        t_term(b, c, j, k, y_dns_2, z_dns_2),
                        v_term(k, r, c, s, z_dns_2, x_dns_2),
                        A_term(a, r, x_dns_2),
                        B_term(s, i, x_dns_2),
                    )
                    # noinspection SpellCheckingInspection
                    R_double_non_symmetric_2 += np.einsum(
                        "aq, qlds, bdji, sl->baji",
                        A_term(a, q, x_dns_2),
                        v_term(q, l, d, s, x_dns_2, z_dns_2),
                        t_term(b, d, j, i, y_dns_2, x_dns_2),
                        B_term(s, l, z_dns_2),
                    )
                    # noinspection SpellCheckingInspection
                    R_double_non_symmetric_2 -= np.einsum(
                        "baki, lkrp, pj, rl->baji",
                        t_term(b, a, k, i, y_dns_2, x_dns_2),
                        v_term(l, k, r, p, z_dns_2, y_dns_2),
                        B_term(p, j, y_dns_2),
                        B_term(r, l, z_dns_2),
                    )
...
def residual_double_total(x_d: int, y_d: int) -> np.array:
    ...
    return (
        residual_double_sym(x_d, y_d)
        + residual_double_non_sym_1(x_d, y_d)
        + residual_double_non_sym_2(x_d, y_d)
    )
...
def update_one(r_1_value: np.array) -> np.array:
    ...
    update_1 = np.zeros((a, i), dtype=np.complex128)
...
def t_non_periodic():
    ...
    # print(np.allclose(single_new, single, atol=1e-20))
    # print(np.allclose(double_new, double, atol=1e-20))
    ...
    # print(np.allclose(t_a_i_tensor, tensors.t_a_i_tensor, atol=1e-20))
    # print(np.allclose(t_ab_ij_tensor, tensors.t_ab_ij_tensor, atol=1e-20))
    ...
    while True:
        ...
        if np.isnan(single).any() or np.isnan(double).any():
